...
# ion_hash
RUN git clone https://github.com/ioncoincore/ion_hash ion_hash
RUN cd ion_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} ion
RUN useradd -u ${USER_ID} -g ion -s /bin/bash -m -d /ion ion

# Extra packages
ARG BUILD_TARGET=linux64
ADD ci/matrix.sh /tmp/matrix.sh
RUN . /tmp/matrix.sh && \
...
RUN mkdir /ion-src && \
  ...
  chown $USER_ID:$GROUP_ID /ion-src && \
