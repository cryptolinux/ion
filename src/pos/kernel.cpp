...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    if (pindexPrev->nHeight == 0) {
        ...
        fGeneratedStakeModifier = true;
        nStakeModifier =  Params().GetConsensus().nFirstBlockStakeModifier; //uint64_t("stakemodifier");
        return true;
    }
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    ...
    if (CXIonStake* xION = dynamic_cast<CXIonStake*>(stake)) {
        ...
        if (depth < Params().GetConsensus().nZerocoinRequiredStakeDepth)
            return error("%s : xION stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
...
bool initStakeInput(const CBlock block, std::unique_ptr<CIonStake>& ionStake, std::unique_ptr<CXIonStake>& xionStake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        xionStake.reset(new CXIonStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, xionStake.get()))
...
{
    std::unique_ptr<CIonStake> ionStake;
    std::unique_ptr<CXIonStake> xionStake;
    ...
    if(!initStakeInput(block, ionStake, xionStake, nPreviousBlockHeight))
        return error("%s : stake input object initialization failed", __func__);

    if (!ionStake && !xionStake) {
    ...
    // Kernel (input 0) must match the stake hash target per coin age (nBits)
    const CTxIn& txin = tx->vin[0];
    CBlockIndex* pindexfrom = ionStake ? ionStake->GetIndexFrom() : xionStake->GetIndexFrom();
    if (!pindexfrom)
    ...
    if (!CheckStakeKernelHash(pindex->pprev, block.nBits, ionStake ? (CStakeInput*)(ionStake.get()) : (CStakeInput*)(xionStake.get()), nTxTime, hashProofOfStake, true)) {
