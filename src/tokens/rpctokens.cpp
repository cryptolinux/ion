...
UniValue createrawtokentransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the ion address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
            "      \"data\": \"hex\"      (string, required) The key is \"data\", the value is hex encoded data\n"
            "      ,...\n"
            "    }\n"
            "3. \"token_outputs\"         (string, required) a json object with addresses as keys and a json objects with the ION and tokens to send\n"
            "    {\n"
            "      \"address\":           (numeric, required) The key is the ION address, the value is a json object with an ION amount, tokengroup ID and token value as values\n"
            "      {\n"
            "        \"amount\":\"x.xxx\"       (numeric, required) The ION amount\n"
