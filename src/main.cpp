...
#if defined(NDEBUG)
#error "ION cannot be compiled without assertions."
#endif
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        // Skip signature verification during initial block download, or if blockheight is smaller than zerocoin v2 startheight
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (GetAdjustedTime() > GetSporkValue(SPORK_9_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
    ...
    if (!tx.IsCoinBase() && !tx.IsZerocoinSpend()) {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (coins->IsCoinBase() || coins->IsCoinStake()) {
                if (nSpendHeight - coins->nHeight < (nSpendHeight <= 100 ? (int)10 : Params().COINBASE_MATURITY()))
...
void RecalculateXIONSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from xION supply
...
bool RecalculateIONSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (block.nTime > GetSporkValue(SPORK_9_ZEROCOIN_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsZerocoins()) {
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateIONSupply(Params().Zerocoin_StartHeight());
    }

    //Track xION money supply in the block index
    if (!UpdateXIONSupply(block, pindex, fJustCheck))
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent xION spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of xION serial %s in block\n Block: %s",
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().MidasStartHeight() && block.GetBlockTime() <= pindexPrev->GetMedianTimePast()) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsXION() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ION: We use certain sporks during IBD, so check to see if they are
        // available. If not, ask the first peer connected for them.
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_8_NEW_PROTOCOL_ENFORCEMENT) &&
                !pSporkDB->SporkExists(SPORK_9_ZEROCOIN_MAINTENANCE_MODE);
...
int ActiveProtocol()
{
    // SPORK_8 is used for 70913 (v3.1.0+)
