...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            " - \"ion\" activates all Ion-specific categories at once;\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "ion")
            + HelpExampleRpc("debug", "ion+net")
        );
    ...
    if (request.fHelp || request.params.size() != 2) {
        ...
        if(sporkManager.UpdateSpork(nSporkID, nValue, *g_connman)){
            ...
            throw std::runtime_error(
                ...
                "\nExamples:\n"
                + HelpExampleCli("spork", "SPORK_12_INSTANTSEND_ENABLED 4070908800")
                + HelpExampleRpc("spork", "\"SPORK_12_INSTANTSEND_ENABLED\", 4070908800"));
        }
    }
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given ion address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The ion address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The ion address validated\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"")
            + HelpExampleRpc("validateaddress", "\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"")
        );
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "to the wallet before upgrading to v0.17. To use the deprecated functionality, start iond with -deprecatedrpc=createmultisig\n"
            "\nArguments:\n"
    ...
    for (unsigned int i = 0; i < keys.size(); ++i) {
        if (IsHex(keys[i].get_str()) && (keys[i].get_str().length() == 66 || keys[i].get_str().length() == 130)) {
...
#endif
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("Invalid public key: %s\nNote that from v5.00, createmultisig no longer accepts addresses."
            ...
            " To use the deprecated functionality, start iond with -deprecatedrpc=createmultisig", keys[i].get_str()));
        }
    }
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The ion address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\", \"signature\", \"my message\"")
        );
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressmempool", "'{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}'")
            + HelpExampleRpc("getaddressmempool", "{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}")
        );
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressutxos", "'{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}'")
            + HelpExampleRpc("getaddressutxos", "{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}")
        );
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressdeltas", "'{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}'")
            + HelpExampleRpc("getaddressdeltas", "{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}")
        );
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddressbalance", "'{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}'")
            + HelpExampleRpc("getaddressbalance", "{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}")
        );
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "]\n"
            "\nExamples:\n"
            + HelpExampleCli("getaddresstxids", "'{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}'")
            + HelpExampleRpc("getaddresstxids", "{\"addresses\": [\"idFcVh28YpxoCdJhiVjmsUn1Cq9rpJ6KP6\"]}")
        );
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "ion-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        "getinfo\n"
        "\nThis call was removed in version 5.00.0. Use the appropriate fields from:\n"
        ...
        "\nion-cli has the option -getinfo to collect and format these in the old format."
    );
}
