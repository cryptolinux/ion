...
bool IsOldBudgetBlockValueValid(const CBlock& block, int nBlockHeight, CBlockReward blockReward, const CAmount coinstakeValueIn, std::string& strErrorRet) {
    ...
    CBlockReward rewardInBlock(block, nBlockHeight >= consensusParams.POSPOWStartHeight && block.IsProofOfWork(), coinstakeValueIn);
...
*
*   Why is this needed?
*   - In Ion some blocks are superblocks, which output much higher amounts of coins
...
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    CBlockReward rewardsInBlock(block, nBlockHeight >= consensusParams.POSPOWStartHeight && block.IsProofOfWork(), coinstakeValueIn);
    ...
    if (nBlockHeight < consensusParams.nBudgetPaymentsStartBlock) {
        ...
        if(!isBlockRewardValueMet) {
            strErrorRet = strprintf("coinbase pays too much at height %d (actual=%d vs limit=%d), exceeded block reward, old budgets are not activated yet",
                                    nBlockHeight, rewardsInBlock.GetTotalRewards().IONAmount, blockReward.GetTotalRewards().IONAmount);
        }
        return isBlockRewardValueMet;
    ...
    CAmount nSuperblockMaxValue =  blockReward.GetTotalRewards().IONAmount + CSuperblock::GetPaymentsLimit(nBlockHeight);
...
void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CBlockReward blockReward, std::vector<CTxOut>& voutMasternodePaymentsRet, std::vector<CTxOut>& voutSuperblockPaymentsRet)
{
    ...
    LogPrint(BCLog::MNPAYMENTS, "%s -- nBlockHeight %d blockReward %lld voutMasternodePaymentsRet \"%s\" txNew %s", __func__,
                            nBlockHeight, blockReward.GetTotalRewards().IONAmount, voutMasternodeStr, txNew.ToString());
}
...
bool CMasternodePayments::GetMasternodeTxOuts(int nBlockHeight, CBlockReward blockReward, std::vector<CTxOut>& voutMasternodePaymentsRet) const
{
...
bool CMasternodePayments::GetBlockTxOuts(int nBlockHeight, CBlockReward blockReward, std::vector<CTxOut>& voutMasternodePaymentsRet) const
{
    voutMasternodePaymentsRet.clear();
