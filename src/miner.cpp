...
bool BlockAssembler::SplitCoinstakeVouts(std::shared_ptr<CMutableTransaction> coinstakeTx) {
    ...
    if (!vpwallets[0]) {
        ...
        if (nValue / 2 > (CAmount)(vpwallets[0]->nStakeSplitThreshold * COIN)) {
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn,
                                    std::shared_ptr<CMutableTransaction> pCoinstakeTx, std::shared_ptr<CStakeInput> coinstakeInput)
{
    CBasicKeyStore tempKeystore;
#ifdef ENABLE_WALLET
    const CKeyStore& keystore = !vpwallets[0] ? tempKeystore : *vpwallets[0];
#else
    ...
    if (fPos) {
        pblock->nTime = pCoinstakeTx->nTime;
    } else {
