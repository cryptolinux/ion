...
void CGovernanceVote::Relay(CConnman& connman) const
{
    ...
    // When this vote is from non-valid (PoSe banned) MN, we should only announce it to v4.0.0.1 nodes as older nodes
...
bool CGovernanceVote::Sign(const CKey& key, const CKeyID& keyID)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_14_NEW_SIGS)) {
