...
bool GenerateKeyPair(const CBigNum& bnGroupOrder, const arith_uint256& nPrivkey, CKey& key, CBigNum& bnSerial)
{
    ...
    arith_uint256 hashPubKey = UintToArith256(Hash(pubKey.begin(), pubKey.end()));
    ...
    arith_uint256 nBits = hashPubKey >> 248; // must be less than 0x0D to be valid serial range
    if (nBits > 12)
        return false;

    //Mark this as v2 by starting with 0xF
    arith_uint256 nMark = 0xF;
    nMark <<= 252;
    hashPubKey |= nMark;
...
void PrivateCoin::mintCoin(const CoinDenomination denomination) {
    ...
    for(uint32_t attempt = 0; attempt < MAX_COINMINT_ATTEMPTS; attempt++) {
        ...
        bool isValid = false;
        while (!isValid) {
            isValid = GenerateKeyPair(this->params->coinCommitmentGroup.groupOrder, arith_uint256(0), key, s);
        }
...
int ExtractVersionFromSerial(const CBigNum& bnSerial)
{
    try {
        ...
        arith_uint256 nMark = bnSerial.getuint256() >> (256 - PrivateCoin::V2_BITSHIFT);
        if (nMark == 0xf)
...
{
    arith_uint256 serial = bnSerial.getuint256();
