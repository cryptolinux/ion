    ...
    unsigned int resultLen)
{
    ...
    uint256 sighash = SignatureHash(priorScript, tx, inputIdx, nHashType, inputAmount, SIGVERSION_BASE);
    ...
    int txbuflen)
{
    ...
    smd->checker = std::make_shared<TransactionSignatureChecker>(smd->tx.get(), inputIdx, inputAmount);
    smd->sm = new ScriptMachine(flags, *smd->checker, 0xffffffff);
    return (void *)smd;
}
