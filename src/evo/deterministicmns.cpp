// Copyright (c) 2018-2019 The XXXXXXX developers
...
bool CDeterministicMNManager::BuildNewListFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, CValidationState& _state, CDeterministicMNList& mnListRet, bool debugLogs)
{
    ...
    for (int i = block.IsProofOfStake() ? 2 : 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
            ...
            Coin coin;
            if (!proTx.collateralOutpoint.hash.IsNull() && (!GetUTXOCoin(dmn->collateralOutpoint, coin) || coin.out.nValue != MASTERNODE_COLLATERAL_AMOUNT)) {
